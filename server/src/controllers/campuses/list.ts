...
const handler = async (
  req: Request<{}, {}, CampusesAPITypes.ListRequestBody>,
  res: Response<CampusesAPITypes.ListResponseData>,
  _next: NextFunction,
) => {
  ...
  try {
    ...
    const fieldsToPopulate = populate
      ? getFieldsToPopulate<
        CampusInclude,
        NonNullable<CampusesAPITypes.ListRequestBody["populate"]>
      >(populate, {
    ...
    if (error instanceof Error) {
      LoggingService.log({
        ...
        level: "error",
        traceId: req.traceId,
        message: "Unexpected error during account campus listing",
