...
type CompleteEnrollmentParameters = {
  enrollmentId: string;
};
...
export class EnrollmentAlreadyCompletedError extends Error {
  retryable = false;
  constructor(message = "enrollment-already-completed") {
    super(message);
    this.name = "EnrollmentAlreadyCompletedError";
  }
}
...
export async function completeEnrollment(
  params: CompleteEnrollmentParameters,
  options: CompleteEnrollmentOptions = {},
): Promise<Enrollment> {
  ...
  try {
    ...
    LoggingService.log({
      source: "services:enrollment:complete",
      level: "important",
...
export async function completeEnrollmentWithRetry(
  params: CompleteEnrollmentParameters,
  options: CompleteEnrollmentOptions = {},
): Promise<Enrollment> {
  return retry(
    async (bail, attempt) => {
      const startTime = performance.now();
      try {
        return await completeEnrollment(params, options);
      } catch (error: any) {
        if (
          error instanceof EnrollmentNotFoundError ||
          error instanceof EnrollmentAlreadyCompletedError ||
        ...
        LoggingService.log({
          source: "services:enrollment:complete:retry",
          level: "warning",
          traceId: options.traceId,
          duration: Number((performance.now() - startTime).toFixed(3)),
          message: `Retryable error during enrollment completion (attempt ${attempt})`,
          details: {
            error: error?.message,
            stack: error?.stack,
          },
